...
// TODO : remove traded offers form alive_buy and alive_sell

bool isValid(std::unordered_map<std::string, long long> alive_buy,
			std::unordered_map<std::string, long long> alive_sell,
			std::string order_name, long long amount, char buy_sell )
{	if(!alive_buy.contains(order_name))
		alive_buy[order_name] = INVALID;

	if (!alive_sell.contains(order_name))
		alive_sell[order_name] = INVALID;
	
	if (buy_sell == 'b')
	{
		if (alive_buy[order_name] == INVALID && alive_sell[order_name] == INVALID)
		{	
			return true;
		}
		else if (alive_buy[order_name] != INVALID && alive_buy[order_name] >= amount)
		{	
			return false;
		}
		else if (alive_sell[order_name] == amount)
		{
			// TODO : Remove alive buy/sell offer from lin_combs
			alive_sell[order_name] = INVALID;
			alive_buy[order_name] = INVALID;
			...
			// TODO : Remove alive buy offer from lin_combs
			alive_buy[order_name] = INVALID;
			return true;
		}
		...
		if (alive_sell[order_name] == INVALID && alive_buy[order_name] == INVALID)
		{	
			return true;
		}
		else if (alive_sell[order_name] != INVALID && alive_sell[order_name] <= amount)
		{	
			return false;
		}
		else if (alive_buy[order_name] == amount)
		{
			// TODO : Remove alive buy/sell offer from lin_combs
			alive_sell[order_name] = INVALID;
			alive_buy[order_name] = INVALID;
		...
		std::unordered_map<std::string, long long> alive_sell)
{
	...
	while (true) {
		...
		char        buy_sell;
		int         idx = -1;
		std::vector<std::string> order_stock_names;
		...
		do {
			...
			order_stock_names.push_back(stock_name);
			lin_combs[num_alive][stock_name] = quantity;

			DEBUG_MSG(stock_name << '\t' << quantity << '\n');
		} while (curr[idx + 1] - '9' > 0);
		
		// Sorting Stock names in alphabetical order
		std::sort(order_stock_names.begin(), order_stock_names.end());
		
		// Storing Sorted order/line name
		for (const std::string& w : order_stock_names) {
        	order_name += w + " ";
    	}
		...
		if (!isValid(alive_buy, alive_sell, order_name, amount, buy_sell))
		{	DEBUG_MSG("No Trade\n");
			std::cout << "No Trade\n";
			// TODO : Remove this offer from lincombs and prices and also num_alive--
		}
		else{
			...
			if (arbitrage(num_alive, lin_combs, prices, arbitrages)) {
				...
				// No Arbitrage found so this is alive order
				if (buy_sell == 'b')
					alive_buy[order_name] = amount;
				else
					alive_sell[order_name] = amount;
			}
		}
		num_alive++;
		begin = end + 2;
...
void part_2(Receiver rcv)
{
	...
	while (!input_end) {
		...
		part_2_sol(message, sp, num_alive, lin_combs, prices, profit, alive_buy, alive_sell);
